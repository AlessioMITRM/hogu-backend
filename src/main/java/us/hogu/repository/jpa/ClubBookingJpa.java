...
public interface ClubBookingJpa extends JpaRepository<ClubBooking, Long> {
       ...
       // FORNITORE - Prenotazioni future (da oggi in poi)
       @Query("SELECT cb FROM ClubBooking cb WHERE cb.clubService.id = :clubServiceId AND cb.reservationTime >= :now")
       Page<ClubBooking> findFutureBookings(Long clubServiceId, OffsetDateTime now, Pageable pageable);

       // FORNITORE - Prenotazioni passate (fino a ieri)
       @Query("SELECT cb FROM ClubBooking cb WHERE cb.clubService.id = :clubServiceId AND cb.reservationTime < :now")
       Page<ClubBooking> findPastBookings(Long clubServiceId, OffsetDateTime now, Pageable pageable);

       // FORNITORE - Prenotazioni ricevute per un club (dal JSON: GET
       // /bookings?providerId=)
       ...
       List<ClubBooking> findByClubServiceAndDateRange(Long clubServiceId, OffsetDateTime startDate,
       ...
       List<ClubBooking> findConflictingBookings(Long clubServiceId, OffsetDateTime reservationTime,
       ...
       @Query("SELECT cb FROM ClubBooking cb " +
                     "JOIN FETCH cb.eventClubService e " +
                     "WHERE cb.status IN :statuses")
       List<ClubBooking> findAllByStatusInWithEvent(@org.springframework.data.repository.query.Param("statuses") java.util.Collection<us.hogu.model.enums.BookingStatus> statuses);
}
